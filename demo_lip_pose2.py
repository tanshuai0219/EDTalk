...
class Demo(nn.Module):
    def __init__(self, args):
        ...
        if args.need_crop_source_img:
            if is_video_file(args.source_path) == False:
                ...
                crop_source_path = os.path.join(os.path.dirname(args.source_path), 'crop_'+os.path.basename(args.source_path))
                crop_cmd = f"python data_preprocess/crop_video.py --inp {args.source_path} --outp {crop_source_path}"
                os.system(crop_cmd)

                args.source_path = crop_source_path
        ...
        if is_video_file(args.source_path) == False:
            ...
            self.img_source, self.fps = vid_preprocessing(args.source_path, args.size).cuda()
        ...
        if self.args.fix_pose == False:
            ...
            self.pose_vid_target, _ = vid_preprocessing(args.pose_driving_path)
