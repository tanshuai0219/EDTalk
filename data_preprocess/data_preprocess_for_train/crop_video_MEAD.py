...
MEAD_dir = 'MEAD_front/Original_video'
save_MEAD_dir = 'MEAD_front/video'


all_data = sorted(os.listdir(MEAD_dir))
print(len(all_data))
...
def process_single_video(name, args):
    prefix = name.split('.')[0]
    source_path = os.path.join(MEAD_dir, name)
    target_path = os.path.join(save_MEAD_dir, prefix + '.mp4')
